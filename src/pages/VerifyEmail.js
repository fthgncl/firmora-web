        ...
        const verifyEmail = async () => {
            try {
                ...
                setResult(
                    error.response?.data || {
                        status: 'error',
                        message: t('verifyEmail:errors.generic'),
                    }
                );
            } finally {
                setLoading(false);
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        <IconWrapper status="loading">
                            <Box sx={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                ...
                                <CircularProgress size={80} sx={{ color: 'white', position: 'absolute', opacity: 0.8 }} />
                            </Box>
                        </IconWrapper>
                ...
                <Fade in={!loading}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        ...
                        <Stack direction="row" spacing={2} sx={{ mt: 3 }}>
                            <ActionButton variant="outlined" startIcon={<HomeIcon />} onClick={() => navigate('/')}>
                                {t('verifyEmail:buttons.home')}
                            </ActionButton>
                            <ActionButton variant="contained" startIcon={<LoginIcon />} onClick={() => navigate('/sign-in')}>
        ...
        return (
            <Fade in={!loading}>
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    ...
                    <StatusMessage variant="body1" color="text.secondary">
                        {result?.message || t('verifyEmail:success.defaultMessage')}
                    </StatusMessage>
                    <ActionButton
                        variant="contained"
                        size="large"
